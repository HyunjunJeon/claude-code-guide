이 페이지는 MCP 서버가 워크플로우 도중에 사용자에게 구조화된 입력을 요청하는 Elicitation(v2.1.49+) 기능을 정리한다. 자유 형식 prompt가 아니라 "옵션 선택", "필수 필드 채우기", "확인" 같은 폼-style 요청을 안전하게 처리해야 할 때 본다. hook을 통한 자동 응답·검증 패턴까지 포함하므로 [06-hooks/](https://wikidocs.net/344613) 모듈과 함께 읽어도 좋다.
