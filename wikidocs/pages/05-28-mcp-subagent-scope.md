이 페이지는 agent frontmatter의 `mcpServers:` 키로 특정 subagent에게만 MCP 서버를 노출하는 방법을 다룬다. 한두 개 agent만 특정 외부 도구가 필요할 때 전체 프로젝트에 굳이 노출하지 않으려는 격리 전략으로 본다. subagent 자체 설계는 04. Subagents, 프로젝트/사용자 범위와의 비교는 mcp-scopes.md에서 다룬다.
