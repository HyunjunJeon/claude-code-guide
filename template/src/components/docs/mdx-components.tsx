...
function createAnchor(moduleSlug?: string) {
  return function Anchor({ children, href, ...props }: React.ComponentProps<"a">) {
    const resolved = href ? rewriteLink(href, moduleSlug) : href;
...
export function createMdxComponents(moduleSlug?: string): MDXComponents {
  return {
    pre: CodeBlock,
    code: InlineCode,
    table: Table,
