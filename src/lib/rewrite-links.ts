...
export function rewriteLink(href: string, currentModule?: string, lang: "ko" = "ko"): string {
  ...
  if (!href || href.startsWith("http") || href.startsWith("#") || href.startsWith("mailto:") || href.startsWith(`/${lang}/`)) {
    return href;
  }
  // Handle legacy /modules/ links (rewrite to include lang)
  ...
  if (crossModuleMatch) {
    return `${prefix}/${crossModuleMatch[1]}${hash}`;
  }
  ...
  if (crossModuleSubMatch) {
    return `${prefix}/${crossModuleSubMatch[1]}/${crossModuleSubMatch[2]}${hash}`;
  }

  // Pattern: ./README.md (same module root)
  if (path === "./README.md" || path === "README.md") {
    return currentModule ? `${prefix}/${currentModule}${hash}` : href;
  }
  ...
  if (sameModuleMatch) {
    return currentModule
      ? `${prefix}/${currentModule}/${sameModuleMatch[1]}${hash}`
      : href;
  }
  ...
  if (bareMatch) {
    return currentModule
      ? `${prefix}/${currentModule}/${bareMatch[1]}${hash}`
      : href;
  }
