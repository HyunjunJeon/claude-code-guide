...
이 페이지는 plugin이 자체 MCP 서버를 번들해 설치만으로 자동 활성화되도록 하는 방법(`.mcp.json` 또는 `plugin.json` 인라인)을 정리한다. plugin을 만드는 입장에서 MCP 도구까지 함께 배포하고 싶을 때 본다. plugin 자체에 대한 더 큰 그림은 [07. Plugins](../07-plugins/README.md), 일반적인 MCP 서버 등록 방식은 [mcp-installation.md](mcp-installation.md)에서 다룬다.
