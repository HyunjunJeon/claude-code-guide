...
이는 본 가이드의 [코드 실행으로 컨텍스트 팽창 해결](#코드-실행으로-컨텍스트-팽창-해결) 원칙과도 같은 맥락이다. MCP 서버가 무엇을 반환하든 한 번에 다 받지 말고 토큰 단위로 끊어서 가져오는 것이 운영의 정석이다.
...
- [Claude Code CLI 참조](https://code.claude.com/docs/ko/cli-reference)
