...
이 페이지는 IT 관리자가 `managed-mcp.json`으로 조직 전체에 MCP 서버 정책(allowlist, denylist, 명령/URL 패턴 매칭)을 강제하는 방법을 정리한다. 사용자나 프로젝트 설정보다 위에서 동작하는 게이트라서 기업 배포·보안 감사 시 가장 먼저 확인해야 할 페이지다. 일반 사용자 범위 설정은 [mcp-scopes.md](mcp-scopes.md), 운영 명령은 [mcp-management.md](mcp-management.md)에서 다룬다.
