...
이 페이지는 MCP를 더 깊게 파고들 때 참고할 외부 공식 문서·블로그·도구 링크를 모아 둔다. 모듈에서 다룬 내용을 넘어서 프로토콜 spec이나 Anthropic 엔지니어링 블로그 원문을 보고 싶을 때 시작점으로 본다. 가이드 내부 문서 흐름은 모듈 메인 [README.md](README.md)의 "이 장의 핵심 주제" 표를 따라가면 된다.
