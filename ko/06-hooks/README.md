...
Hooks는 Claude Code에서 특정 이벤트가 발생할 때 자동으로 실행되는 자동화된 동작(셸 명령, HTTP 웹훅, LLM 프롬프트 또는 subagent 평가)입니다. JSON 입력을 받고 종료 코드와 JSON 출력을 통해 결과를 전달합니다.

**주요 기능:**
- 이벤트 기반 자동화
- JSON 기반 입출력
- command, prompt, HTTP, agent hook 타입 지원
- 도구별 hook을 위한 패턴 매칭
...
Claude Code는 **26개의 hook 이벤트**를 지원합니다:
...
| **TaskCreated** | TaskCreate로 작업 생성 | (없음) | 아니오 | 작업 추적, 로깅 |
...
| **PreCompact** | 컨텍스트 압축 전 | manual/auto | 아니오 | 압축 전 동작 |
...
| **비동기 hook** | 독립적으로 계속 실행되며 gating control flow에는 적합하지 않음 |
...
- **[공식 Hooks 문서](https://code.claude.com/docs/en/hooks)** - 전체 hooks 참조
