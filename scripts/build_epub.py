...
class EPUBConfig:
    ...
    ko_title: str = "Claude Code 사용 가이드"
    ko_subtitle: str = "한 주말에 Claude Code 마스터하기"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="en",
        choices=["en", "ko"],
        help="Language code: 'en' for English, 'ko' for Korean (default: en)",
    )
    parser.add_argument(
        "--puppeteer-config",
        type=str,
    ...
    lang_map: dict[str, tuple[Path, str, str]] = {
        "en": (repo_root, "claude-code-guide.epub", EPUBConfig.en_title),
        "ko": (repo_root / "ko", "claude-code-guide-ko.epub", EPUBConfig.ko_title),
    }
